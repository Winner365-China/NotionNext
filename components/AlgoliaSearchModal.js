    ...
    >
      ...
      >
        <div className="flex justify-between items-center">
          <div className="text-2xl text-blue-600 dark:text-yellow-600 font-bold">关键词搜索</div>
          <div>
            <i
        ...
        </div>
        {
          searchResults.length === 0 && keyword && !isLoading && (
            <div>
              <p className=" text-slate-600 text-center my-4 text-base"> 找不到有关<span className='font-semibold' >&quot;{keyword}&quot;</span>的结果，可更换关键词后重试</p>
            </div>
          )
        }
        ...
        <div className='flex items-center justify-between mt-2 sm:text-sm text-xs dark:text-gray-300'>
          ...
          <div>
            {totalHit > 0 && (
              <p>
                搜索到 {totalHit} 条结果，用时 {useTime} 毫秒
              </p>
            )}
          </div>
          <div className="text-gray-600 dark:text-gray-300  text-right">
            <span>
               Search by <a href="https://www.algolia.com/?utm_source=instantsearch.js&utm_medium=website&utm_content=blog.365sites.top&utm_campaign=poweredby" rel="noopener noreferrer"  target="_blank"><i className="fa-brands fa-algolia"></i> Algolia</a>
            </span>
          </div>
        </div>
